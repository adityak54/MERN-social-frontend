...
const Post = ({
  ...
  return (
    <div className="bg-black text-white p-4  rounded-lg shadow-lg m-5 relative min-w-[600px]">
      {postedBy && (
        <div className="text-sm text-white flex flex-row gap-3">
          Posted by:
          <span> {postedBy.toUpperCase()}</span>
        </div>
      )}
      {postfilter === "my" && (
        <div
          onClick={() => setOpen(!open)}
          className=" absolute top-2 right-2 hover:cursor-pointer font-bold py-2 px-4 rounded"
        >
          ...
          <div>
            {open && (
      ...
      <div className="flex justify-between mt-4">
        ...
        <div className="flex justify-end items-center relative">
          ...
          <div>
            {commentopen && (
              <div className="absolute bottom-5  right-5  bg-gray-700 p-2 rounded-lg shadow-lg border border-black flex flex-col gap-3">
                <form onSubmit={handleCommentSubmit}>
                  <input
                    type="text"
                    className="bg-white border-black text-black border-[1px] rounded  p-2 mb-4"
                  ></input>
